...
from models import Generator, MLP, Discriminator
...
def train_dmd(cfg, teacher, diffusion):
    ...
    opt_D = optim.Adam(D.parameters(), lr=cfg.dmd_lr_D)
    opt_fake = optim.Adam(fake_model.parameters(), lr=cfg.dmd_lr_fake)
    scheduler_G = torch.optim.lr_scheduler.CosineAnnealingLR(
        opt_G, T_max=cfg.dmd_epochs, eta_min=1e-6
    )
    scheduler_D = torch.optim.lr_scheduler.ConstantLR(opt_D, factor=1.0)  # 常数
    scheduler_fake = torch.optim.lr_scheduler.LinearLR(
        opt_fake, start_factor=1.0, end_factor=0.1, total_iters=cfg.dmd_epochs
    )
    # 损失函数（用于 fake 模型）
    mse = nn.MSELoss()
    ...
    for epoch in range(cfg.dmd_epochs):
        ...
        D.train()
        fake_model.eval()
        for _ in range(1,4):  # 判别器更新次数可调
            # 真实样本
            #x_real = sample_real_data(batch_size)
            t = torch.randint(0, cfg.T, (cfg.batch_size,),device = cfg.device)
            try:
                z_ref, y_ref = next(paired_iter)
            except StopIteration:
                paired_iter = iter(paired_loader)
                z_ref, y_ref = next(paired_iter)
            x_real = y_ref.to(cfg.device)  # 形状 (batch_size, data_dim)
             # --- 从生成器采样一批假样本 ---
            with torch.no_grad():  # 判别器训练时，生成器不更新
                z_fake = torch.randn(cfg.batch_size, cfg.data_dim, device=cfg.device)
                x_fake = G(z_fake)  # 形状 (batch_size, data_dim)
    
            # 对真实样本加噪
            x_t_real, _ = diffusion.q_sample(x_real, t)
    
            # 对假样本加噪
            x_t_fake, _ = diffusion.q_sample(x_fake, t)

            logits_real = D(x_t_real, t)

            logits_fake = D(x_t_fake, t)
